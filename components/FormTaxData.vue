		...
		<v-row no-gutters>
			<v-col cols="12">
				<validation-observer ref="observer" v-slot="{ invalid }">
					<v-form id="form-tax-data" @submit.prevent="sendForm">
						<v-row>
							...
							<v-col class="py-0" cols="12">
								...
								>
									...
									>
										<v-radio
											v-for="(option, i) in $store.state.identity
												.vatTypeOptions"
											:key="i"
											:value="option.id"
										>
						...
						<v-row v-if="!hideSubmitButton" class="mt-2 mt-md-4">
	...
	methods: {
		...
		async sendForm() {
			if (await this.$refs.observer.validate()) {
				...
				this.$store
					.dispatch('identity/updateBillingData', {
						data: this.form,
						$nuxt: this.$nuxt,
					})
					.finally(() => {
